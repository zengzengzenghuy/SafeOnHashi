...
abstract contract ShuSo is OwnableUpgradeable {
    ...
    error DuplicateOrOutOfOrderAdapters(
        address emitter,
        IOracleAdapter adapterOne,
        IOracleAdapter adapterTwo
    );
    ...
    function init(bytes memory initParams) public initializer {
        (address _owner, Hashi _hashi) = abi.decode(
            initParams,
            (address, Hashi)
        );
        __Ownable_init();
    ...
    function _setHashi(Hashi _hashi) internal onlyOwner {
        if (hashi == _hashi)
            revert DuplicateHashiAddress(address(this), _hashi);
        hashi = _hashi;
    ...
    function _setThreshold(
        ...
        if (domains[domain].threshold == threshold)
            revert DuplicateThreashold(address(this), threshold);
    ...
    /// @notice Only callable by the owner of this contract.
    function _enableOracleAdapters(
        uint256 domain,
    ...
    /// @notice Reverts if _adapters are out of order or contain duplicates.
    /// @notice Only callable by the owner of this contract.
    function _disableOracleAdapters(
        uint256 domain,
        IOracleAdapter[] memory _adapters
    ) internal onlyOwner {
        if (domains[domain].count == 0)
            revert NoAdaptersEnabled(address(this), domain);
        if (_adapters.length == 0) revert NoAdaptersGiven(address(this));
        for (uint256 i = 0; i < _adapters.length; i++) {
            IOracleAdapter adapter = _adapters[i];
            if (adapter == IOracleAdapter(address(0)) || adapter == LIST_END)
            ...
            if (current.next == IOracleAdapter(address(0)))
    ...
    function getOracleAdapters(
        uint256 domain
    ) public view returns (IOracleAdapter[] memory) {
        IOracleAdapter[] memory _adapters = new IOracleAdapter[](
            domains[domain].count
        );
    ...
    function getThresholdAndCount(
        uint256 domain
    ) public view returns (uint256 threshold, uint256 count) {
    ...
    function checkAdapterOrderAndValidity(
        ...
        for (uint256 i = 0; i < _adapters.length; i++) {
            ...
            if (i > 0 && adapter <= _adapters[i - 1])
                revert DuplicateOrOutOfOrderAdapters(
                    address(this),
                    adapter,
                    _adapters[i - 1]
                );
    ...
    function _getThresholdHash(
        ...
        for (uint i = 0; i < _adapters.length; i++) {
            try _adapters[i].getHashFromOracle(domain, id) returns (
                bytes32 currentHash
            ) {
